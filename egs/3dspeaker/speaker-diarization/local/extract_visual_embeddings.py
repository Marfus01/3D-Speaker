...
def main():
    ...
    # 将 vad.json 的内容按所属video分组，整理为 dict 格式的 vad_data，key(str) 是video文件名，value(dict) 是与该video对应的所有 visual segment info(包含 id, start, stop, filepath)
    ...
    for vpath in videos:
        ...
        for key in vad_json:
            k = str(key)
            if k.rsplit('-', 2)[0]==rec_id:
                subset[key] = vad_json[key]
        assert len(subset) > 0, "[ERROR] No vad info found in %s for %s." %(args.vad, vpath)
        vad_data[rec_id] = subset

    # 将 subseg.json 的内容按所属video分组，整理为 dict 格式的 subseg_data，key(str) 是video文件名，value(dict) 是与该video对应的所有 audio segment info(包含 id, start, stop, filepath)
    with open(args.subseg, "r") as f:
        subseg_json = json.load(f) # record the time regions which need to be extracted in each video
    ...
    for vpath in videos:
        ...
        assert len(subset) > 0, "[ERROR] No subseg info found in %s for %s." %(args.subseg, vpath)
        subseg_data[rec_id] = subset
    ...
    for vpath in local_videos:
        # get filename without suffix of current video and corresponding audio path
