...
为了在face level而非frame level计算acc，需要完成：
1. 聚类结果保存为json，key为face id = f"{audio_segment_id}_{face_idxs}"，value为聚类簇 id。
2. 筛选聚类结果中存在于标注数据集中的face id，计算acc。
> 由于标注数据集上在人脸检测结果的基础上构建的，因此如果一帧有标注数据，其标注应该与簇标签一一对应。
### 人脸模态聚类
#### 与avd的对齐
1. 人脸部分，active speaker face 与来自中间帧的人脸的绝对数量基本相当，但是前者质量较高，内容相对单一；后者则存在区域不完整、误检、模糊等问题。因此，可以考虑
a. 先分别做聚类，然后将后者与前者对齐；
> a.1 由于active speaker face和中间帧人脸质量存在差异，分别聚类时，后者层次聚类的停止阈值fix_cos_thr可能需要重新调整。
> a.2 在确定合适的fix_cos_thr后，需要比较align前和不同的align相似度阈值下，根据聚类中心align之后人脸聚类的效果。之所以使用聚类中心，是因为这里在试图确定cluster之间的对应关系，使用聚类中心更鲁棒。
b. 只对前者做聚类，然后最近邻分配后者的label。
> 有两种方式分配后者的label：针对active speaker face的聚类簇中心 or 距离最近的active speaker face所属的聚类簇中心。
> 需要比较不同的align相似度阈值下，使用这两种方式分配label后的人脸聚类效果。
c. 将两者合并做聚类，得到聚类结果后，仅使用active speaker face部分与说话人簇id对齐。
> 需要同时检查active speaker face部分在区分speaker身份方面的质量，和中间帧人脸部分在区分人脸身份方面的质量。
> fix_cos_thr可能需要重新调整
### 与nested_HMM的接口
筛选关键帧人脸聚类结果与说话人align的部分，构建$\hat{F}$。每一张关键帧人脸应该都有一个簇id（与asf都不相似的就扔到others）里
