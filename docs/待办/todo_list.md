...
#### 函数实现优化✅
在开发nested_hmm_x的gpu版本时，发现了一些可以通过广播机制提升cpu版本效率的方式。目前已经迁移到了nested_hmm_x的cpu中。
#### 降低计算复杂度的思路
...
1. 给定t时刻人脸存在状态的观测，定义一个所有人脸存在情况集合的依赖于观测的子集，其余情况均认为后验概率为0。子集的具体定义方式有若干种。例如：
a. 利用embedding cos-sim的信息：对于当前帧的每一张人脸，认为与其余弦相似度最高的top-k个 avd 簇，均有可能是其真实标签；
b. 仅利用人脸存在向量的信息：将0-1 encoding首先转化为簇id的集合，在2^n种可能情况中，筛选对应集合与观测集合重叠度（例如，通过IOU衡量）较高的部分。但这种方法的一个局限性是，当人脸存在向量全0/只有1个1时，IOU指标的阈值难以处理。而且会出现当前帧没有人脸，但hidden states解码出人脸的情况。
2. 将根据face states是否存在于子集中定义的随机变量作为X_f, 影响初始概率和转移概率的计算（主要影响的是归一化步骤）
> 由此构建的人脸转移概率约束效果可能与混淆矩阵约束类似，但在数学上更合理，同时有助于减少计算量
3. U, V统计量的计算中，仍将两者初始化为形状为(T, 2^n, n)，取值均为-inf（对应概率为0）的张量。以U统计量为例
...
其他一些需要修改的部分，可以参考在往nested_hmm增加协变量X时，增加的操作
