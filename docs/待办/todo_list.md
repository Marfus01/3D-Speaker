...
> **选择2**：微调后直接分类 vs 重新聚类。经过比较，前面效果更好（预测结果能与训练伪标签持平甚至略高），而后者聚类簇变化较大，acc只能达到 80%。在论文中可以简要说明这一点。
...
> 后续report 的acc会与之前有轻微不一致，因为前面的acc是基于全部标注数据计算的，自监督微调时，test set只包含80%的标注数据。
