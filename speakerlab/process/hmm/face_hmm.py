...
class FaceHMM:
    ...
    def _validate(F_hat, lengths):
        ...
        if (F_hat.ndim != 2 or 0 in F_hat.shape
                or any(not np.isin(channel, [0, 1]).all() for channel in F_hat.T)):
    ...
    def predict(self, F_hat, lengths=None):
        ...
        decoded = np.empty(F_hat.shape, dtype=np.uint8)
        for j, model in enumerate(self.models_):
            channel = np.asarray(F_hat[:, j:j + 1], dtype=int)
            decoded[:, j] = model.predict(channel, lengths)
        return decoded
