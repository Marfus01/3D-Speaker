...
class NestedHMM_full():
    ...
    def _enumerate_face_configs(self):
        ...
        for i in range(self.n_face_states):
            ...
            for j in range(self.n_actors):
                config.append((i >> j) & 1) # 提取i的二进制表示中的第j位
    ...
    def _compute_face_transition_probs(self, F_idxs_potential_prev=None, F_idxs_potential_curr=None):
        ...
        log_probs = log_probs - logsumexp(log_probs, axis=1)[:, None]  # 归一化
        return log_probs
        ...
        for t in range(n_samples):
            ...
            if t > 0:
                ...
                if F_idxs_curr_key not in stats_updated['face_transition_counts']:
                    stats_updated['face_transition_counts'][F_idxs_curr_key] = [F_idxs_prev, face_transition_weights_filtered]
                else: ### 合并已有的转移统计量
                    ...
                    if set(F_idxs_prev_old) == set(F_idxs_prev):
                        ...
                        stats_updated['face_transition_counts'][F_idxs_curr_key] = [F_idxs_prev, face_transition_weights_filtered_new]
                    else:
            ...
            face_states = np.array([0, 1])  # true face state
            ...
            face_emission_counts_delta = (gamma_faces_filtered[:, None, None] * mask).sum(axis=0)  # (n_actors, 2)
            stats_updated['face_emission_counts'][actor_idxs[:, None], face_states[None, :], F_hat[t][:, None]] += face_emission_counts_delta   # update all stats_updated['face_emission_counts'][actor, face_state, F_hat[t, actor]] at once. actor \in [0, n_actors-1], face_state \in [0,1]
